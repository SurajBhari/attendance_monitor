...
async def end(ctx):
    if ctx.author != bot.owner:
        return
    await ctx.send(
        "Are you sure you want to end the current semester right here and delete all the previous records ?"
    )
...
async def on_message(message):
    ...
    if message.author.id == config["webhook_id"]:
        ...
        await message.channel.send(
            content="Successfully logged your input", embed=embed
        )
